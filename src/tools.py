...
class GetFieldStatisticsArgs(BaseModel):
    """Retrieve compact field statistics from a Qlik Sense application."""
...
class GetHypercubeSummaryArgs(BaseModel):
    """Retrieve hypercube metadata and summary statistics for a Qlik object."""
    ...
    object_id: Annotated[str, Field(
        description="Visualization object ID exposing qHyperCube metadata.",
        min_length=1,
        max_length=255,
    )]
    max_data_rows: Annotated[int, Field(
        default=1000,
        ge=0,
        le=10000,
        description="Maximum rows to inspect when sampling hypercube data for density statistics.",
    )] = 1000

    @field_validator("app_id", "object_id")
    @classmethod
    def validate_ids(cls, v: str) -> str:
        """Ensure identifiers are not empty and properly formatted."""
        if not v.strip():
...
async def get_field_statistics(
    ...
    """Retrieve compact field statistics focused on cardinality, flags, and table coverage."""
    client = QlikClient()
...
async def get_hypercube_summary(
    app_id: str,
    object_id: str,
    max_data_rows: int = 1000,
) -> dict[str, Any]:
    """Retrieve qHyperCube metadata plus size and density statistics for an object."""
    client = QlikClient()
    ...
    try:
        ...
        result = client.get_hypercube_summary(object_id=object_id, max_data_rows=max_data_rows)
